...
- [ ] **ADX (趨勢強度過濾)**: 最優先實作。計算 +DI 與 -DI，若 ADX < 20 則判定為盤整，大幅降低/遮蔽 MACD 等遲滯指標的干擾，避免被雙巴。
- [ ] **ATR (動態波動率防護)**: 替換當前固定的停損/停利百分比。將停損設為動態的 N 倍 ATR，在極端波動（插針）時自動拓寬安全帶。
- [ ] **EMA Slope (動能衰竭預判)**: 計算均線斜率（加速度），不等 EMA 交叉，一旦斜率放緩即提早平倉入袋。
