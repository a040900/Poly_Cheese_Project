...
1. **優先開發 ADX (趨勢強度過濾)**: 用以偵測短線震盪盤 (ADX < 20)，主動隔離遲滯指標（如 MACD 的假交叉）避免雙巴。
2. **ATR (動態安全閥)**: 拋棄定額停損，在市場波動加劇時拉寬保護傘，減少被惡意插針掃出場的機率。
3. **EMA Slope (動能預警)**: 把「看線交叉」轉化為「看拋物線斜率」，只要斜率衰竭立刻啟動提早獲利了結。
